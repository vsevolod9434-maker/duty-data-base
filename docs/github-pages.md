...
Первый восстановительный срез защищённых операций описан в
[`docs/supabase-edge-functions.md`](./supabase-edge-functions.md). Если
`NEXT_PUBLIC_ACCESS_ADMIN_FUNCTION_URL` задан, GitHub Pages клиент отправляет
создание пользователя состава, сброс чужого пароля и управление доступом в Edge
Function `access-admin`. Если переменная не задана или функция недоступна,
интерфейс сохраняет мягкую блокировку защищённых операций.
